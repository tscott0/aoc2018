...
pub fn solve(input: String) -> u32 {
    ...
    // Remove infinite
    let mut removals: Vec<Coord> = vec![];
    for y in min_y - margin..=max_y + margin {
        for x in min_x - margin..=max_x + margin {
            let mut closest: Option<(Coord, i64, bool)> = None;
            ...
            if let Some((r, _, tie)) = closest {
                ...
                {
                    if debug {
                        println!("{:?} was eliminated, found {},{} was at boundry", r, x, y);
                    }
                    removals.push(r);
                }
            }
    ...
    println!("{} locations remain", locs.len());
